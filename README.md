...
4. Chọn mẫu phù hợp (ví dụ: **Node.js**). Chọn phiên bản Node bạn muốn sử dụng.
...
## Docker — build và chạy bằng Docker

Mình đã thêm một `Dockerfile` cho `client/` (multi-stage build: build React app, serve bằng `nginx`) và file `docker-compose.yml` ở root để chạy nhanh.
